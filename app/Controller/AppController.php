...
//                $this->loadModel('ProjectUser');
                ...
                $getallproj = $this->ProjectUser->query("SELECT DISTINCT Project.id,Project.uniq_id,Project.name,Project.default_assign,Project.project_methodology_id,ProjectMethodology.project_template_view_id,ProjectUser.dt_visited FROM project_users AS ProjectUser,projects AS Project LEFT JOIN project_methodologies AS ProjectMethodology ON (Project.project_methodology_id = ProjectMethodology.id ) WHERE Project.id= ProjectUser.project_id AND ProjectUser.company_id='" . SES_COMP . "' AND ProjectUser.user_id=" . SES_ID . " AND Project.isactive='1' AND Project.company_id='" . SES_COMP . "' ORDER BY ProjectUser.dt_visited DESC");
                $getallprojForAdmin = $this->Project->query("SELECT DISTINCT Project.id,Project.uniq_id,Project.name,ProjectUser.dt_visited FROM projects AS Project LEFT JOIN project_users AS ProjectUser ON (ProjectUser.project_id=Project.id)  WHERE Project.isactive='1' AND Project.company_id='" . SES_COMP . "' AND ProjectUser.company_id='" . SES_COMP . "' ORDER BY ProjectUser.dt_visited DESC");
