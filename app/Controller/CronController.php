...
class CronController extends AppController{
    public $name = 'Cron';
    public $components = array('Format','Sendgrid','Postcase','Tmzone','PhpMailer');

    function beforeFilter(){
        if (php_sapi_name() != "cli" && $this->action != 'test_cron' && $this->action != 'test_email') {
            print "Oops! You are not authorized to access this information.";
            exit;
        }
        ...
        $this->Auth->allow('update_email');//over due & upcomming email
    ...
    function email_notification(){
        ...
        $cancelledUser = $CompanyUser->find('list', array("conditions" => array('CompanyUser.company_id' => $cancelled,'CompanyUser.is_active !=' =>1), 'fields' => 'CompanyUser.user_id'));
        ...
        if(!empty($cancelledUser)){
            $getAllNot = $UserNotification->find('all', array('conditions' => array('UserNotification.type' => 1, 'UserNotification.value !=' => 0, 'NOT'=>array('UserNotification.user_id' => $cancelledUser))));
        }else{
            $getAllNot = $UserNotification->find('all', array('conditions' => array('UserNotification.type' => 1, 'UserNotification.value !=' => 0)));
        }
            ...
            foreach($getAllNot as $usr){
				...
				if($usr['UserNotification']['value'] == 1 || ($usr['UserNotification']['value'] == 2 && $day == "Fri") || ($usr['UserNotification']['value'] == 3 && $lastDate == GMT_DATE)) {
					$userStatus = $this->checkUserStatus($usr['UserNotification']['user_id']);
					if(!empty($userStatus)){
...
//echo "<pre>";print_r($userInfo);//exit;
						...
						$message = "<table cellpadding='0' cellspacing='0' align='left' style='width:100%'>
												...
												<tr>
														<td align='left' style='font:10px Arial;padding-top:2px;color:#737373'>
																You are receiving this email notification because you have subscribed to Orangescrum Task Status E-mail notification, to unsubscribe, please click <a href='".$comp_url."/users/email_notifications' target='_blank'>Unsubscribe Email Notification</a>
														</td>	  
												</tr>
										</table>";
								//$to = "mahavir.senapati@andolasoft.co.in";
								if (!defined('CRON_DISPATCHER')) {
										$this->Sendgrid->sendGridEmail(FROM_EMAIL_EC,$to,$subject,$message,"notification");
										echo "To: ".$to;
										echo "<br/>";
										echo $subject;
										echo "<br/>";
										...
										//$subject.=" - CRON";
										//$to = "prakash.satpathy@andolasoft.com";
										$this->Sendgrid->sendGridEmail(FROM_EMAIL_EC_NOTIFY,$to,$subject,$message,"notification");
								}
						}
					}
				}
    ...
    function update_email(){
			...
			foreach($getAllNot as $emlNot) {			
				$userStatus = $this->checkUserStatus($emlNot['UserNotification']['user_id']);
				if(!empty($userStatus)){
					...
					if(count($case_cming) || count($case_late)){
							if(count($case_late) != 0){
								$str_case="<tr><td><table cellpadding='0' cellspacing='0' style='font:bold 14px Arial;width:100%;'><tr style='background:#5191bd;height:50px'><td style='font:bold 14px Arial;padding:10px;color:#ffffff'>Overdue tasks</td></tr><tr><td><table style='border:1px solid #ccc; border-radius:5px;width:100%;border-collapse:collapse;font-size:14px;font-family:Arial;box-shadow:0px 0px 7px #ccc;' cellpadding='4' cellspacing='4'><tr><td style='border-bottom:1px solid #ccc'>Task#</td><td style='border-bottom:1px solid #ccc'>Title</td><td style='border-bottom:1px solid #ccc'>Status</td><td style='border-bottom:1px solid #ccc'>Due Date</td><td style='border-bottom:1px solid #ccc;width: 90px;'>Project</td><td style='border-bottom:1px solid #ccc;width: 90px;'>Late By</td></tr>";
								...
								foreach($case_late as $case){
									...
									$str_case .= "<tr><td align='right' style='padding-right:10px;width:70px;'>$case_no</td><td>$case_title</td><td>$case_status</td><td>$due_dt</td><td style='padding-left:10px;'>$pj_name</td><td style='padding-left:10px;'>$late_dt</td></tr>";
								}
							...
							if(count($case_cming) != 0){ 
								$str_case_cming="<tr><td><table cellpadding='0' cellspacing='0' style='font:bold 14px Arial;width:100%;'><tr style='background:#5191bd;height:50px'><td style='font:bold 14px Arial;padding:10px;color:#ffffff'>Upcoming tasks</td></tr><tr><td><table style='border:1px solid #ccc; border-radius:5px;width:100%;border-collapse:collapse;font-size:14px;font-family:Arial;box-shadow:0px 0px 7px #ccc;' cellpadding='4' cellspacing='4'><tr><td style='border-bottom:1px solid #ccc'>Task#</td><td style='border-bottom:1px solid #ccc'>Title</td><td style='border-bottom:1px solid #ccc'>Status</td><td style='border-bottom:1px solid #ccc'>Due Date</td><td style='border-bottom:1px solid #ccc;width: 90px;'>Project</td><td style='border-bottom:1px solid #ccc;width: 90px;'>Coming up in</td></tr>";
                  ...
                  $due_dt=$this->Format->dateFormatReverse($case['Easycase']['due_date']);
									...
									$pjnam = $this->Project->find('first', array('conditions'=>array('Project.id' => $case['Easycase']['project_id']), 'fields' => array('Project.name')));
									$pj_name=$pjnam['Project']['name'];
									$str_case_cming .= "<tr><td align='right' style='padding-right:10px;width:70px;'>$case_no</td><td>$case_title</td><td>$case_status</td><td>$due_dt</td><td style='padding-left:10px;'>$pj_name</td><td style='padding-left:10px;'> $cming_day</td></tr>";
								}
							...
							$message = "<table cellpadding='0' cellspacing='0' align='left' style='width:100%;'>
														...
														<tr>
																<td align='left'>
																		<table cellpadding='4' cellspacing='4' style='margin:10px 0;width:100%;'>
																						".$str_case."
																		</table>
																</td>
														</tr>
														...
														<tr style='height:25px;'><td>&nbsp;</td></tr>
														<tr>
																<td align='left' style='font:11px Arial;padding-top:5px;color:#737373' colspan='2'>
														...
														<tr>
																<td align='left' style='font:10px Arial;padding-top:2px;color:#737373'>
																You are receiving this email notification because you have subscribed to Orangescrum, to unsubscribe, please click <a href='".$comp_url."/users/email_notifications' target='_blank'>Unsubscribe Email Notification</a>
														</td>
														</tr>
												</table>";
    ...
    function checkUserStatus($user_id){
        $this->loadModel('User');
        $checkUsers = $this->User->find('all',array('joins'=>array(
                array('table'=>'company_users',
                    'alias' => 'CompanyUser',
                                                'type'=>'inner',
                                                'conditions'=>array('User.id=CompanyUser.user_id','CompanyUser.is_active'=>1,'CompanyUser.user_id'=>$user_id))),
                    'fields'=>"CompanyUser.company_id,CompanyUser.is_active,User.id,User.name,User.uniq_id,User.last_name,User.email,User.dt_created,User.timezone_id",'order'=>"User.id DESC"));
        return $checkUsers;
    ...
    function dailyupdate_notifications(){
            ...
            foreach($getAllNot as $emlNot) {
				$userStatus = $this->checkUserStatus($emlNot['DailyupdateNotification']['user_id']);
				if(!empty($userStatus)){
                                    ...
                                    if($query_All) {
                                            ...
                                            $message = "<tr><td><table style='border-collapse:collapse;border-spacing:0;text-align:left;width:600px;border:1px solid #5191BD'>
                                                    ...
                                                    <table style='margin-top:5px;width:600px;'>
                                                            <tr><td style='font:13px Arial;color:#737373;'>Don't want these emails? To unsubscribe, please click <a href='".$comp_url."/users/email_notifications' target='_blank'>Unsubscribe</a> and trun off <b>Daily Update Report</b> E-mail notification.</td></tr>
                                                    </table></td></tr>
                                                    ";
                                                    ...
                                                    if (!defined('CRON_DISPATCHER')) {
                                                            ...
                                                            //$subject.=" - CRON";
                                                            /*$fp = fopen('testcron_dailyupdate.txt','w');
                                                            fwrite($fp,$to);
                                                            fwrite($fp,"\r\n");
                                                            fwrite($fp,$subject);
                                                            fwrite($fp,"\r\n");
					...
					if($value['DailyUpdate']['cron_email_date'] != $dt['0']){//Send an email once a day. Check current date
						...
						if(in_array(date('l',strtotime($locDT)), $days)){//Check weekdays.
							if(strtotime($time) >= strtotime($value['DailyUpdate']['notification_time'])){
								...
								$project = $this->Project->getProjectFields(array('Project.id'=>$value['DailyUpdate']['project_id'],'Project.isactive'=>1),array('id','name','short_name','uniq_id'));
								if(!empty($project)){
									$usr = explode(",",$value['DailyUpdate']['user_id']);//Getting user ids.
									foreach($usr as $key1 => $value1) 
									{
										$userStatus = $this->checkUserStatus($value1);
										if(!empty($userStatus)){
    ...
    function getLinks($user = NULL, $seo_url = NULL) {
	    ...
	    $helpLink = "<a href='http://helpdesk.orangescrum.com/' target='_blank'>HELP</a>";
 ...
 * @return void Sends email to the respective users
 */
    function weeklyusagedetails(){//echo "<pre>";
            ...
            foreach($user_details AS $key=>$val){
                    ...
                    $task_upd_statistics = '<div style="float:left">
                            <div style="width:190px;padding:10px 0;text-align:center;background:#F8F8F8;border-top:1px solid '.$task_upd_color.';border-left:4px solid #FFF">
                                    ...
                                    <div style="clear:both"></div>
                                    <div style="color:#666666;margin-top:5px;font-weight:bold">updates and activities</div>
                                    <div style="clear:both"></div>';
                    if($progress_flag){			
            ...
            <div><ul><li><b>'.$userlogin[0][0]['notlogged'].'</b> Out of <b>'.$userlogin[0][0]['tot'].'</b> User has not logged in to the system since last week.
                            ...
                            if (!defined('CRON_DISPATCHER')) {
                                            ...
                                            //$subject.=" - CRON";
                                            $to =$val['User']['email'];
                                            //if($val['Company']['id']==1 || $val['Company']['id']==62){
                                                    //$to = "prakash.satpathy@andolasoft.com";
                                                    //$to = "gayadhar.khilar@andolasoft.co.in";
                                                    $mail_body = $message_top.$header.$statistics_div.$message;
                                                    $this->Sendgrid->sendGridEmail(FROM_EMAIL,$to,$subject,$mail_body,"usagedetails");
                                            //}
                                    }
                            //$this->Sendgrid->sub_sendgrid(FROM_EMAIL,'andola.gayadhar@gmail.com',$subject,$message);*/
                            //echo $mail_body."<div style='clear:both'><br/><hr/>";//exit;
                    }	
            }
            echo 'success';exit;
    }
    ...
    public function getFirstStatus($custom_sts_id, $lgnd)
    {
        if (!$custom_sts_id) {
            ...
            $grp_dtl =  $CustomStatus->find('first', array('conditions'=>array('CustomStatus.id'=>$custom_sts_id),'order'=>array('CustomStatus.seq'=>'ASC')));
            if ($grp_dtl) {
                $grp_dtl_master =  $CustomStatus->find('first', array('conditions'=>array('CustomStatus.status_group_id'=>$grp_dtl['CustomStatus']['status_group_id']),'order'=>array('CustomStatus.seq'=>'ASC')));
                if ($grp_dtl_master) {
                    return array($grp_dtl_master['CustomStatus']['status_master_id'], $grp_dtl_master['CustomStatus']['id']);
                }
            }
            return array($lgnd, $custom_sts_id);
        }
    }
	function checkForOtherFields($oldEid, $newEid, $proj_id){
		$this->loadModel('EasycaseMilestone');
		$chkMIl = $this->EasycaseMilestone->find('first', array('conditions' => array('EasycaseMilestone.easycase_id' =>$oldEid, 'EasycaseMilestone.project_id' => $proj_id)));
		if($chkMIl){
			unset($chkMIl['EasycaseMilestone']['id']);
			$chkMIl['EasycaseMilestone']['easycase_id'] = $newEid;
			$this->EasycaseMilestone->saveAll($chkMIl);
		}
	}
}
